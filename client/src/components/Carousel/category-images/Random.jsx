import React from "react";
 
export default function Shark(props) {
    return (
        <svg className={props.classNames} xmlns="http://www.w3.org/2000/svg" version="1.1" data-icon="random" data-container-transform="translate(0 3)" viewBox="0 0 32 32" x="0px" y="0px">
            <path d="M26.125.219c-.075.037-.125.131-.125.281v2.5h-2.5c-1.1 0-1.996.371-2.75 1.125l-4.813 4.813 2.125 2.125 4.813-4.813.031-.031c.091-.061.5-.219.594-.219h2.5v2.5c0 .3.206.387.406.188l5.188-3.875c.2-.2.2-.425 0-.625l-5.188-3.875c-.1-.1-.206-.131-.281-.094zm-26.125 2.781v3h5.5c.1 0 .579.204.625.25l14.625 14.625c.754.754 1.65 1.125 2.75 1.125h2.5v2.5c0 .3.206.387.406.188l5.188-3.875c.2-.2.2-.425 0-.625l-5.188-3.875c-.2-.2-.406-.113-.406.188v2.5h-2.5c-.1 0-.579-.204-.625-.25l-14.625-14.625c-.754-.754-1.65-1.125-2.75-1.125h-5.5zm10.938 10.938l-4.813 4.813c.042-.042-.725.25-.625.25h-5.5v3h5.5c1.085 0 1.889-.279 2.719-1.094l.031-.031 4.813-4.813-2.125-2.125z" transform="translate(0 3)"/>
        </svg>
    )
};